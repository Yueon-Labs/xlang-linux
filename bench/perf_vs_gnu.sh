...
# grep: "word" matches every line (worst case for the output path — N matches).
race "grep word"    build/perf_in.txt     "$B/grep word"      "grep word"
race "grep -n word" build/perf_in.txt     "$B/grep -n word"   "grep -n word"
race "grep -c word" build/perf_in.txt     "$B/grep -c word"   "grep -c word"
